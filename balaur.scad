...
module draw() /****** built from the bottom up *******/
{
	...
	raiseit = folded ? Yaxis_seperation+2 : 0; 
		...
		translate([0, i*rodY, Zfloor])
		{
			translate([0, i*-15, NEMA_length(nemaTypeZ)])
			{
				...
				rotate([0,0,i==-1 ? 180 : 0])
					motor_mount_plate(rodYoffset-brace_wall_thickness);
...
module motor_mount_plate(ZrodYoffset)
{
		...
		union()
		{
			...
			difference()
			{
				translate([0, ZmotorWidth/2+2.35-brace_wall_thickness/2, 20/2+plate_height])
					cube(size=[ZmotorWidth, brace_wall_thickness, 20], center=true); 
